...
type ExamineFS() =
    ...
    member this.OnExaminableBodyEntered(body:Node) =
        (this:>IExaminable).OnExaminableBodyEntered(body)
    ...
    override this._Ready() =
        ...
        this.Log.debug [|
            "InteractableArea connection to Area BodyEntered "          
            this.InteractableArea.Connect(Signals.Area.BodyEntered, this, nameof this.OnExaminableBodyEntered)
        |]
        this.Log.debug [|
            "InteractableArea connection to Area BodyExited "          
            this.InteractableArea.Connect(Signals.Area.BodyExited, this, nameof this.OnExaminableBodyExited)
        |]
        
    override this._Process(_delta) =
        if this.CanInteract && InputUtil.IsInteractionJustPressed() then
            this.CanInteract <- false
            
            (this.DialogManager :> IDialogManager).StartDialog 
